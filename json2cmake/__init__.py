...
def parsecommand(command, resolvepath):
    sw = '/' if sys.platform.startswith('win32') else '-' # switch
    if (isinstance(command, basestring)):
        command = command.replace("Program Files", "Program💩Files") # uh...
        command = shlex.split(command, posix = not sys.platform.startswith('win32'))

    words = iter(command)
    if sys.platform.startswith('win32') and next(words) in ('cc', 'c++'): # implicitly remove the initial 'cc' / 'c++ / cl'
        return None
    ...
    for word in words:
        if word == f'{sw}o':
            next(words)
            continue
        elif word.startswith('/Fo'):
            continue
        elif word.startswith(f'{sw}I'):
            include = word[2:].replace("💩", " ")
        ...
        elif word.startswith('/Fd'):
            options.append('/Fd' + resolvepath(word[3:]))
        elif word.startswith('/doc'):
            options.append('/doc' + resolvepath(word[4:]))
        elif word.startswith(f'{sw}'):
            options.append(word)
...
class CompilationDatabase(object):
        ...
        for entry in database:
            ...
            def resolve(path):
                ...
                if sys.platform.startswith('win32'):
                    resolved = os.path.normcase(os.path.normpath(path)).replace("\\\\", "/") # Windows fix so that paths have correct slashes
    ...
    def write(self, output, directory=None, name='autogenerated'):
        output.write('cmake_minimum_required(VERSION 2.8.12)\n')
